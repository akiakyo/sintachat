...
function fmt(value:number){return `${String(Math.floor(value/60)).padStart(2,"0")}:${String(value%60).padStart(2,"0")}`}
  ...
  return <div className="conversation-page">
          ...
          return <div className={`message-row ${message.mine?"mine":"theirs"} ${actionsOpen?"actions-open":""}`} key={message.id}>
            ...
            <div className="bubble-wrap" style={{transform:`translateX(${offset}px)`}}>
              <div className="bubble-cluster">
                ...
                >
                  {message.replyTo&&<div className="reply-snippet" aria-label="Quoted message"><span>{message.replyTo.text}</span></div>}
    ...
    {reportOpen&&<div className="report-chat-backdrop" onClick={()=>setReportOpen(false)}><section className="report-chat-card" onClick={e=>e.stopPropagation()}><div className="report-chat-head"><div><small>SAFETY CHECK</small><b>Report this conversation</b><span>Your report stays private.</span></div><button aria-label="Close report" onClick={()=>setReportOpen(false)}>×</button></div><p className="report-chat-note">Tell us what happened. Reports use anonymous conversation and session IDs only.</p><label><span>What should we review?</span><select value={reportReason} onChange={e=>setReportReason(e.target.value)}><option>Harassment</option><option>Sexual Content</option><option>Threats</option><option>Spam</option><option>Impersonation</option><option>Hate / Abuse</option><option>Other</option></select></label><label><span>More context <em>optional</em></span><textarea value={reportDetails} onChange={e=>setReportDetails(e.target.value)} maxLength={500} placeholder="Share only what helps us understand the situation."/></label><div className="report-chat-meta"><span>{reportDetails.length}/500</span><small>We review reports to help keep chats respectful.</small></div><button className="report-submit" onClick={submitReport}>{reportStatus?.startsWith("Report submitted")?"Report sent":"Send private report"}</button>{reportStatus&&<small className={`report-status ${reportStatus.startsWith("Report submitted")?"success":""}`}>{reportStatus}</small>}</section></div>}
